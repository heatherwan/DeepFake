...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators, netD_A(check B), netD_B (check A)
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        # define parameters,losses and optimizers
        if self.isTrain:
    ...
    def set_input(self, inputs):
        ...
        if self.opt.face_mask:
            self.mask_A = inputs['A_mask' if AtoB else 'B_mask'].to(self.device)
            self.mask_B = inputs['B_mask' if AtoB else 'A_mask'].to(self.device)
    ...
    def backward_G(self):
        ...
        # GAN loss D_A(G_A(A)) D_B(G_B(B)) # prediction and expect True
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B), True)
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)

        # cycle (reconstruction) loss || G_B(G_A(A)) - A|| || G_A(G_B(B)) - B||
        if not self.opt.face_mask:
            ...
            self.loss_cycle_A = ((self.rec_A - self.real_A).abs() * self.mask_A).mean() * self.opt.lambda_A
            self.loss_cycle_B = ((self.rec_B - self.real_B).abs() * self.mask_B).mean() * self.opt.lambda_B
